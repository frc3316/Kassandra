...
class GoalsScored(StatsHandler):
    ...
    def _run_stats(success, failure):
        ...
        stats['size'] = DeffencesCrossed.AMOUNT_GRANULATOR.get(total_shots)
        stats['color'] = DeffencesCrossed.PERCENTAGE_GRANULATOR.get(total_scored / total_shots)
        stats['variant'] = DeffencesCrossed.HIGH_VARIANCE < GoalsScored._variance(
                [float(s) / (s + f) for (s, f) in zip(success, failure)])
...
class DeffencesCrossed(StatsHandler):
    ...
    def _run_stats(success, failure):
        ...
        stats['variant'] = DeffencesCrossed.HIGH_VARIANCE < DeffencesCrossed._variance(
                [float(s) / (s + f) for (s, f) in zip(success, failure)])
    ...
    def filter(self, match_data):
        ...
        breaching_success = {deffence: [match[deffence]['success'] for match in breaching_data] for deffence in
                             DEFFENCES}
        breaching_failure = {deffence: [match[deffence]['failure'] for match in breaching_data] for deffence in
                             DEFFENCES}
        return {deffence: DeffencesCrossed._run_stats(breaching_success[deffence], breaching_failure[deffence]) for
                deffence in DEFFENCES}
